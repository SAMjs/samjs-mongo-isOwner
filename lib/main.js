      ...
      isOwner: function(options) {
        ...
        getQuery = (function(_this) {
          return function(user, query) {
            var owner;
            owner = _this.helper.createQuery(name, user[prop]);
            return _this.helper.addToQuery(query, owner);
          };
        })(this);
        return this.addHook("afterCreate", function() {
          ...
          this.addHook("beforePopulate", (function(obj) {
            var i, len, populate, ref, user;
            user = obj.socket.client.auth.user;
            ref = obj.populate;
            for (i = 0, len = ref.length; i < len; i++) {
              populate = ref[i];
              if ((populate.samjsmodel.plugins.isOwner != null) && !isAllowed(user, "read", populate.samjsmodel)) {
